...
class RanderCard {
  ...
  static render_records(records) {
    ...
    for (let record of records) {
      ...
      if (record.isSelected) {
        recordEl.addClass('randerSelectedItem'); //let checkBox = recordEl.find('input[type=checkbox]');
      ...
      $('#randerRecords').append(recordEl); //console.log(recordEl.html());
    }
  ...
  static resultBtnHandler() {
    ...
    var afterRandom = function () {
      hitItem.isSelected = false; //$('#randerRecords').find('[data-item-id="' + hitItem.itemId + '"]').removeClass('selectedItem');

      RanderCard.render_records(RanderCard.records);

      if (selectedItems.length == 1) {// only one item left                
      }
    };
  } //------------------------------------------------------
...
_defineProperty(RanderCard, "records", []);
